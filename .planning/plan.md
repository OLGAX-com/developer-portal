...
- [ ] Gamified onboarding dashboard, task board, and mentor verification — see [.planning/onboarding-and-community-plan.md](./onboarding-and-community-plan.md)
...
- 2026-08-06 — User then asked the bigger question directly: since `developer-portal` (this platform) is itself open source, why isn't IT just another tracked project instead of being special-cased at the docs root? Checked the facts before answering: this workspace had NO git repo at all (`.git` didn't exist), and `OLGAX-com` only had 2 real GitHub repos (`olgax-pos`, `.github`) — `developer-portal` didn't exist on GitHub yet, so it genuinely couldn't be tracked like `olgax-pos` regardless of the architecture question. Explained the real reasoning for keeping platform docs hand-authored at root (primary audience/entry point, richer Fumadocs authoring + search) vs. why the user's point was still valid (copilot-instructions.md literally says this project is open source too, so it should be a trackable `Project` like any other once it has a repo). User created `https://github.com/OLGAX-com/developer-portal` on the spot. Ran `git init -b main`, added the remote, verified via `git check-ignore` that all `.env*` files were correctly excluded before staging (no secrets committed), committed, confirmed with the user, then pushed the first commit. Registered it via the existing `pnpm --filter @olgax/github run add-project` script — same mechanism as `olgax-pos`, zero new code needed. Verified live in-browser: `developer-portal` now appears correctly in both `/docs/projects` (apps/docs) and would appear in `/projects` (apps/web) via the exact same live-sync path, and its real root `docs/plan.md` + `docs/onboarding-and-community-plan.md` show up as real doc pages automatically — validating that the "GitHub is the source of truth, no special-casing" design genuinely holds up once a repo exists. Removed the now-resolved "unconfirmed guess" comment on `apps/docs/lib/shared.ts`'s `gitConfig` since the repo name is now confirmed real. Full lint/typecheck/test pass.
